...
import ru.newsystems.nispro_bot.webservice.services.TelegramRegistrationBotService;

@Route("")
@PageTitle("Telegram registration")
public class TelegramRegistrationView extends VerticalLayout {

    Grid<TelegramBot> grid = new Grid<>(TelegramBot.class);
    TextField filterText = new TextField();
    RegistrationForm registrationForm;
    TelegramRegistrationBotService service;

    public TelegramRegistrationView(TelegramRegistrationBotService service) {
    ...
    private void configureGrid() {
        ...
        grid.setSizeFull();
        grid.setColumns("company", "url", "login", "password", "queueId");
...
//        addContactButton.setMinWidth("250px");
    ...
    public void editContact(TelegramBot telegramBot) {
        if (telegramBot == null) {
            closeEditor();
        } else {
            registrationForm.setTelegramEntity(telegramBot);
    ...
    private void closeEditor() {
        registrationForm.setTelegramEntity(null);
